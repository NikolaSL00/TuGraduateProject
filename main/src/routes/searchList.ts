...
router.post("/api/main/searchList", async (req, res) => {
    ...
    for (const store of stores) {
        ...
        for (const element of listData) {
            ...
            const storeProducts = store.products.filter(product => regex.test(product.title));
            if (storeProducts.length > 0) {
                matchArray.push(storeProducts);
            }
        }
        storeProducts.push({ store: { name: store.name, locations: store.locations }, products: matchArray })
    }
